    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-md:max-w-full max-w-md bg-white dark:bg-gray-800 rounded-2xl shadow-2xl flex flex-col max-h-[85vh] transform transition-all animate-in zoom-in-95 duration-200 overflow-x-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-5 overflow-x-hidden">
           ...
           <div className="space-y-3">
              {items.map((item) => {
                ...
                const daysLeft = Math.ceil((expiryMidnight - nowMidnight) / (1000 * 60 * 60 * 24));
                  ...
                  <div key={item.id} className="flex items-center gap-3 p-3 bg-red-50 dark:bg-red-900/10 rounded-xl border border-red-100 dark:border-red-900/30">
                    <div className="w-12 h-12 bg-gray-200 dark:bg-gray-600 rounded-lg overflow-hidden flex-shrink-0 relative">
                      {coverMedia ? (
                        ...
                        <div className="absolute inset-0 flex items-center justify-center text-gray-400"><ImageIcon size={18} /></div>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-gray-900 dark:text-white truncate">{item.name}</h3>
                      <div className="flex items-center gap-2 mt-0.5">
                         <div className="flex items-center gap-1 text-xs text-orange-600 dark:text-orange-400 font-medium"><CalendarClock size={12} /><span>{formatExpirtyDate(expiryDate)}</span></div>
                         <span className="text-xs font-bold px-1.5 py-0.5 bg-white dark:bg-gray-800 rounded text-red-500 shadow-sm border border-red-100 dark:border-gray-700">{dayText}</span>
                      </div>
                    </div>
                  </div>
                );
        ...
        <div className="p-4 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 rounded-b-2xl">
           <button onClick={onClose} className="w-full py-2.5 bg-gray-900 hover:bg-gray-800 dark:bg-white dark:hover:bg-gray-100 text-white dark:text-gray-900 rounded-xl font-medium transition-colors">{t.dismiss}</button>
        </div>
      </div>
    </div>
  );
